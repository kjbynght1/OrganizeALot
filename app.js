...
const VERSION='v2.1.0 Build 022';
...
async function usePendingPhoto(){
  if(!state.pendingFile||!state.current)return;const rec={id:'photo_'+Date.now()+'_'+Math.random().toString(36).slice(2,8),inspectionId:state.current.id,itemKey:state.pendingItemKey,created:new Date().toISOString(),name:state.pendingFile.name||'photo.jpg',type:state.pendingFile.type||'image/jpeg',blob:state.pendingFile};await dbPut(PHOTO_STORE,rec);state.photos.push(rec);closePhotoModal();await saveCurrent('Photo saved');renderSections();renderInspectionHeader();
}
...
async function exportZip(){
  ...
  const blob=await zip.generateAsync({type:'blob',compression:'DEFLATE',compressionOptions:{level:6}});const filename=`OrganizeALot_${slug(c.inspectionId)}_${slug(c.address)}_Build022.zip`;
